...
async function getTicketDetails(id: string, userId: string) {
...
export default async function UserTicketDetailPage({ params }: { params: { id: string } }) {
  ...
  const ticket = await getTicketDetails(id, (session.user as any).id);

  if (!ticket) {
    return (
      <div className="container mx-auto px-4 py-10 text-center">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Ticket not found</h1>
        <Link href="/dashboard/support" className="text-blue-600 hover:underline">
          Return to Support
        </Link>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
        <div className="px-6 py-6 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900">{ticket.subject}</h1>
            <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
              <span className="flex items-center gap-1">
                <Clock className="w-4 h-4" /> Opened {new Date(ticket.createdAt).toLocaleDateString()}
              </span>
